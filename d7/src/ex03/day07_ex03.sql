...
SELECT
    COALESCE(selected_orders.name, selected_visits.name) AS name,
    COALESCE(selected_orders.count, 0 ) + COALESCE(selected_visits.count, 0) AS total_count
FROM selected_orders
  FULL JOIN selected_visits ON selected_orders.name=selected_visits.name
